...
各社のエージェントや個々のプロジェクトに依存しないような汎用的な指示をグローバル AGENTS.md にまとめました。また、ソースコードの品質を意識してハーネスやスキルも併せて作成しました。
...
最近分かったのですが、下記を記述してると「Generate Commit Message」が機能しなくなる場合があります。一度機能しなくなると数日間機能しなくなります。不要かもです。
