...
- 回答、レスポンス、コード内の説明、思考プロセス、コミットメッセージを含むすべての USER-FACING TEXT は日本語のみで行うこと。
...
- Markdown のテーブル表記 (`|` 区切り表) や ASCII 図・テキスト図では、日本語を使用してはならない。
  ...
  - 本項は、5–7 行目で定義された「英語での回答禁止」規定に対する限定的な例外とし、表・図のセル／ラベルに限って英語の使用を許可する。
...
## ANTIGRAVITY-SPECIFIC RULES (Antigravity 専用規則)
