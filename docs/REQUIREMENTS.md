...
Rust ハーネスは Rust 関連変更がある場合だけ `make check` と `make build` を実行し、成功後は
Rust 関連差分だけを正しさ、回帰、セキュリティ、テストの観点でレビューするよう自然言語で
エージェントへ要求する。Rust 関連差分がない場合と同一の検証済み fingerprint では、標準出力へ
フック応答を出力してはならない。指示は特定の AI エージェント名、専用コマンド、レビュー結果
JSON に依存してはならない。レビューで Rust 関連ファイルが変更された場合は、次の Stop で
検証をやり直す。
