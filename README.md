...
- 後日、人がコードの内容を確認できるよう、コードがどこまで生成されているか確認できるよう、要件定義書や仕様書や設計書を先にさくせうするよう指示しています。
- 新規開発中にエージェントとやりとりする度にドキュメントが生成されないように、上書きせず追記することを指示しています。
...
新規開発フェーズと保守・修正フェーズでコーディングエージェントを区分けするために、「フェーズ別特例」を設けています。ユーザーがどちらのフェーズでコーディングエージェントを呼び出すかによって、適用される方針が変わります。
