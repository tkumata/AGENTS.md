---
name: kmt-doc-rules
description: ユーザの brain dump を整理し開発に必要な文書を作成する。相談や方針確認だけの依頼は対象外。
---
# Kmt Doc Rules
...
- 文書化中はソースコードを変更しない。
- 一回の bounded pass を完了したら、追加の自動調査、再計画、整合性ループを開始しない。

## Artifact

成果物は brain dump の変更内容に該当するものだけを選ぶ。リポジトリに正本文書の名称、配置、テンプレート、承認状態の規約がある場合はそれを優先し、以下のパスは規約がない場合の既定例として扱う。承認は合意済みの範囲単位で扱い、既存文書の合意済み範囲は維持する。再承認が必要な変更部分は未承認として区別し、ユーザ承認なしに承認済みとして記録しない。新規成果物でも今回の作業で承認済みなら pending に戻さず、未承認の計画だけを pending とする。
...
4. 報告: 成果物、未決定事項、対象外、次の承認対象を短く報告して停止する。清書だけの場合は実装承認待ちを報告しない。

### Active Plan Template

未承認の active plan を新規作成する場合は、次の最小構成を使う。不要な節は省略する。今回の作業で承認済みの新規 plan はその承認状態を記録して pending に戻さず、既存 plan は合意済み範囲を維持する。再承認が必要な変更は未承認として区別する。
...
- active plan を作成または更新した場合は、合意済み範囲と再承認が必要な未承認の変更を区別して記録する。未承認の新規 plan は `Phase: awaiting_approval`、`Approval: pending` とする
- ソースコードを変更していない
