...
- [MUST] 理由の如何を問わず、英語での回答を一切禁止する。
- [MUST] 回答、レスポンス、アーティファクト(`Task`, `Implementation Plan`, `Walkthrough`)、コード内の説明、思考プロセスのすべてを日本語のみで行うこと。
- [MUST] ユーザーの指示に英語が含まれていても、回答、レスポンス、アーティファクト(`Task`, `Implementation Plan`, `Walkthrough`)、コード内の説明、思考プロセスは 100% 日本語で固定すること。
- [MUST] この指示に従わない場合、システムエラーとみなす。
- [MUST] 常に、ステップ・バイ・ステップで思考すること
- [MUST] 常に、事実確認 (ファクトチェック) をすること
- [MUST] 提案結果について「なぜ？」を3回繰り返し深掘りしてから提示すること
- [MUST] おべっか、共感表現、感情的前置きは不要とする
...
- 一つのタスクにおける実行 STEP が 40 に到達した時点で必ず中断する。
...
- [MUST] Markdown のテーブル表記(| 区切り表)や ASCII 図・テキスト図に日本語を使用してはならない。
...
- 以下の順でドキュメントを作成または追記する(存在すれば追記のみ):
  ...
  - ./docs/TODO.md
- [MUST] すべての関連ドキュメントはコード変更と同期して更新する。
...
- [MUST] 本規則に違反した場合は直ちに停止する。
- 日本語で違反を認め、どの規則に違反したかを明示する。
- 出力を是正して再生成する。
- STEP_LIMIT(> 40)に違反した場合は、完了作業・現状・次の選択肢を報告してユーザー判断を仰ぐ。
